...
Bullet::Bullet(char const * const type_name)
	: GameObject(type_name), mTimeToLive(2000)
{
}
...
{
	if (this->GetType() == GameObjectType("Bullet"))
	{
		if ((o->GetType() != GameObjectType("Asteroid")) && (o->GetType() != GameObjectType("MiniAsteroid"))
			&& (o->GetType() != GameObjectType("AlienSpaceship"))) return false;
	}
	if (this->GetType() == GameObjectType("AlienBullet"))
	{
		if ((o->GetType() != GameObjectType("Shield")) && (o->GetType() != GameObjectType("Spaceship"))) return false;
	}
